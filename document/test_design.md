...
| A-15 | 著者名が256文字を超える場合は登録できない | 異常系 | 400を返す | 高 |
...
| A-16 | 著者名が256文字を超える場合は更新できない | 異常系 | 400を返す | 高 |
...
| B-31 | 書籍名が256文字を超える場合は登録できない | 異常系 | 400を返す | 高 |
...
| B-32 | 書籍名が256文字を超える場合は更新できない | 異常系 | 400を返す | 高 |
...
| 対象 | テスト内容 | 期待結果 |
|---|---|---|
| AuthorService | 生年月日が未来日の場合は登録できない | `BusinessRuleViolationException` が発生する |
| BookService | PUBLISHEDからUNPUBLISHEDへ更新できない | `BusinessRuleViolationException` が発生する |
| BookService | authorIdsに重複がある場合は登録できない | `BusinessRuleViolationException` が発生する |
| BookService | 存在しない著者IDを指定した書籍登録はできない | `BusinessRuleViolationException` が発生する |

## 10.4 補足

Service層テストでは、RepositoryをMock化して業務ルールの分岐を直接確認する。
著者の生年月日判定は固定Clockを使い、実行日に依存しない形で検証する。
